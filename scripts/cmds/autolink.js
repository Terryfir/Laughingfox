...
const supportedPlatforms = {
    ...
    facebook:
        /^(https?:\/\/)?((?:www|m|web)\.)?(facebook|fb)\.(com|watch)\/.*$/,
    instagram:
...
function checkRateLimit(userId) {
    const now = Date.now();
    const userLimit = userDownloadLimits.get(userId) || {
        count: 0,
        timestamp: now
    };
...
async function getVideoData(url) {
    try {
        ...
        const response = await axios.get(
            `https://dev-priyanshi.onrender.com/api/alldl?url=${encodedUrl}`,
            {
                timeout: 30000,
                headers: {
                    "User-Agent":
...
async function downloadVideo(videoData, sock, event, threadID) {
    try {
        const videoPath = path.join(
            __dirname,
            `temp_video_${threadID}_${Date.now()}.mp4`
        );
...
export default {
    ...
    onRun: async function ({ args, message, event, threadID, senderID }) {
        ...
        if (
            !args[0] ||
            !["on", "off", "status"].includes(args[0].toLowerCase())
        ) {
        ...
        return message.reply(
            `${statusEmoji} Auto download ${statusText} for this chat!\n\n` +
                (command === "on"
                    ? `🎯 Send any video link from: ${Object.keys(
                          supportedPlatforms
                      ).join(", ")}\n` +
