            ...
            for (let site of data) {
                ...
                try {
                    ...
                    if (site.lastStatus === 1) {
                        ...
                        const ownerTag = ownerID.includes('@') ? ownerID.split('@')[0] : "user";

                        try {
                            const ssRes = await axios.get(`https://api.ccprojectsapis-jonell.gleeze.com/api/screenshot?url=${encodeURIComponent(site.url)}`);
                            const ssUrl = ssRes.data.screenshotURL;

                            if (!ssUrl) throw new Error("Screenshot API returned empty");

                            const caption = `🚨 *${font.bold("MONITOR DOWN")}*\n\nAttention @${ownerTag}!\n\nYour website is currently unreachable.\nURL: ${site.url}\nReason: ${error.message}\nUptime Score: ${((site.successChecks / site.totalChecks) * 100).toFixed(2)}%`;
...
export default {
    ...
    async onRun({ message, args, threadID, font, sock, event }) {
        ...
        if (action === "check") {
            ...
            await message.reply("🔍 *Checking status and capturing live view...*");

            const start = Date.now();
            try {
                const res = await axios.get(url, { timeout: 20000 });
                const latency = Date.now() - start;
                const ssRes = await axios.get(`https://api.ccprojectsapis-jonell.gleeze.com/api/screenshot?url=${encodeURIComponent(url)}`);
                const ssUrl = ssRes.data.screenshotURL;

                if (!ssUrl) throw new Error("Could not generate screenshot");

                const checkMsg = `🖥️ *${font.bold("SITE CHECK RESULT")}*\n\n` +
                                 `*URL:* ${url}\n` +
                                 `*Status:* ${res.status} OK ✅\n` +
                                 `*Latency:* ${latency}ms\n` +
                                 `*Owner:* @${senderID.split('@')[0]}`;

                await sock.sendMessage(threadID, { image: { url: ssUrl }, caption: checkMsg, mentions: [senderID] }, { quoted: event });
            } catch (e) {
            ...
            try {
                ...
                await message.reply(`✅ *${font.bold("MONITOR ADDED")}*\n\nNow tracking ${url}.\nOwner: @${senderID.split('@')[0]}`, { mentions: [senderID] });
            } catch (e) {
                return message.reply(`❌ *Failed:* Site is offline or invalid.`);
            }
        } 
        
        else if (action === "list") {
            const list = data.filter(s => s.threadID === threadID);
            if (list.length === 0) return message.reply("No monitors found in this chat.");
            ...
            list.forEach((s, i) => {
                ...
                const uptimePct = ((s.successChecks / s.totalChecks) * 100).toFixed(1);
                const owner = s.senderID ? `@${s.senderID.split('@')[0]}` : "System";
            ...
            const siteIndex = data.findIndex(s => s.url === url && s.threadID === threadID);
            if (siteIndex === -1) return message.reply("URL not found.");
            
            const site = data[siteIndex];
            if (site.senderID && site.senderID !== senderID) {
                return message.reply(`❌ *Access Denied:* Only owner can ${action} this.`);
            }

            if (action === "remove") data.splice(siteIndex, 1);
            else if (action === "pause" || action === "resume") site.isPaused = action === "pause";
            else if (action === "edit") {
                const newUrl = args[2]?.toLowerCase();
                if (!newUrl) return message.reply("Usage: !uptime edit [old] [new]");
                site.url = newUrl;
                site.lastStatus = 1;
                site.totalChecks = 0;
            ...
            await message.reply(`*${font.bold("UPTIME MANAGER")}*\n\n!uptime add [url]\n!uptime check [url]\n!uptime list\n!uptime remove [url]\n!uptime pause/resume [url]\n!uptime edit [old] [new]`);
        }
    }
};