...
export default {
    config: {
        name: "quiz",
        description:
            "Play a quiz game using questions from ALOC, with subject, type, and year filters.",
    ...
    async onRun({ sock, args, message, threadID, event, senderID }) {
        try {
            ...
            if (commandArg === "leaderboard") {
                ...
                if (scores.length === 0) {
                    return await sock.sendMessage(threadID, {
                        text: "🏆 The quiz leaderboard is currently empty. Be the first to score!"
                    });
                }
            ...
            if (!subject) {
                const categoryList = AVAILABLE_SUBJECTS.map(
                    subj => `• ${subj}`
                ).join("\n");
                return await sock.sendMessage(threadID, {
                    text: `📚 Available Quiz Subjects:\n\n${categoryList}\n\nTo start, use: quiz <subject> [type] [year]\n(Example: quiz chemistry utme 2010)\n\nFor help, type: \`quiz help\``
                });
            }

            if (!AVAILABLE_SUBJECTS.includes(subject)) {
                return await sock.sendMessage(threadID, {
                    text: `Subject "${subject}" is not valid. Use \`quiz\` to see available subjects.`
                });
            }
            ...
            const info = `Subject: ${apiResponse.subject.toUpperCase()}${
            ...
            if (error.response) {
                if (error.response.status === 401) {
                    errorText =
                        "❌ Unauthorized: Please ensure your ALOC AccessToken is correct in the command file headers.";
                }
            }
    ...
    async onReply({ sock, event, data, threadID, senderID, args }) {
        ...
        const replyAnswer = (
            typeof args === "string" ? args.split(" ")[0] : args
        ).toUpperCase();

        const currentData = leaderboard[sender] || {
            score: 0,
            name: event.pushName || sender
        };
