...
class MessageHandler {
  ...
  async handleMessage(event) {
    try {
      ...
      const message = {
        send: form => this.sock.sendMessage(threadID, { text: form }),
        reply: form => this.sock.sendMessage(threadID, { text: form }, { quoted: event }),
        edit: (form, data) => this.sock.sendMessage(threadID, { text: form, edit: data.key }),
        react: (emoji, data) => this.sock.sendMessage(threadID, { react: { text: emoji, key: data.key } }),
        unsend: data => this.sock.sendMessage(threadID, { delete: data.key }),
        sendGif: (filepath, cap) => this.sock.sendMessage(threadID, { video: { url: filepath, caption: cap || "", gifPlayback: true } }),
        sendAudio: (filepath, cap) => this.sock.sendMessage(threadID, { audio: { url: filepath, caption: cap || "" } }),
        sendVideo: (cap, filepath, boo) => this.sock.sendMessage(threadID, { image: { url: filepath }, viewOnce: boo || false, caption: cap || "" }),
        sendImage: (cap, filepath, boo) => this.sock.sendMessage(threadID, { image: { url: filepath }, viewOnce: boo || false, caption: cap || "" })
      };
      const bot = {
